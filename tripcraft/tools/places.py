...
class PlaceSearch:
    ...
    async def search(self, city: str, query: str = None, limit: int = 5) -> dict:
        ...
        # Step 2A: Query Geoapify if key is configured (Preferred)
        if self._config.has_geoapify and not warning_message:
            try:
                geo_key = self._config.geoapify_key
                url = "https://api.geoapify.com/v2/places"
                params = {
                    "categories": "tourism,catering,entertainment,heritage,leisure",
                ...
                logger.info(f"Searching Geoapify places in {resolved_city_name} ({lat},{lon}) for query: {query}")
                response = await request_with_retry("GET", url, params=params, timeout=10.0)
                if response.status_code == 200:
                    raw_features = response.json().get("features", [])
                    for feat in raw_features:
                        prop = feat.get("properties", {})
                        plat = prop.get("lat")
                        plon = prop.get("lon")
                        name = prop.get("name") or prop.get("formatted", "Interesting Place")
                        cats = prop.get("categories", ["Attraction"])
                        ...
                        places.append({
                            "name": name,
                            "categories": cats[:3],
                            "address": prop.get("address_line2") or prop.get("formatted") or resolved_city_name,
                            "latitude": plat,
                            "longitude": plon,
                            "maps_link": maps_link,
                ...
                logger.warning(f"Geoapify places search failed: {ex}")

        # Step 2B: Query Foursquare if key is configured and Geoapify did not return results
        if not api_success and self._config.has_foursquare and not warning_message:
            try:
                headers = {
                ...
                logger.warning(f"Foursquare places search failed: {ex}")
