...
class Config:
    def __init__(self):
        ...
        # ── API keys ──
        self.gemini_keys_list = []
        for suffix in ["", "_2", "_3", "_4", "_5", "_6", "_7", "_8", "_9", "_10"]:
            val = os.getenv(f"GEMINI_API_KEY{suffix}")
            if val and val.strip() and val not in self.gemini_keys_list:
                self.gemini_keys_list.append(val)
        self.gemini_key = self.gemini_keys_list[0] if self.gemini_keys_list else ""
